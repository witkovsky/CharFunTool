function [pmf,n,tMin,tMax,cdfFun,pmfFun] = RenewalProcessPMF(t,cf,nMin,nMax,tol)
%RenewalProcessPMF 
%  Evaluates the renewal process PMF(t), the probability mass function
%  evaluated at time t, which is specified by the given characteristic
%  function (CF) of the holding time distribution.
%
%  For each specified time t the renewal process pmf(t) gives the
%  probability mass function defined on the non-negative integers from the
%  specified interval (nMin,nMax). The pmf(t) can be evaluated at arbitrary
%  time t >= 0. However, for correct result t must be from the associated
%  interval (tMin,tMax), which can be specified by the RenewalProcessPMF
%  algorithm.
%
...
%
% SYNTAX:
%  pmf = RenewalProcessPMF(t,cf)
%  [pmf,n] = RenewalProcessPMF(t,cf,nMin,nMax,tol)
%  [pmf,n,tMin,tMax] = RenewalProcessPMF(t,cf,nMin,nMax,tol)
%  [pmf,n,tMin,tMax,cdfFun,pmfFun] = RenewalProcessPMF(t,cf,nMin,nMax,tol)
% 
% INPUTS:
%  t      - scalar value of time t >= 0, where the probability mass
%           function pmf(t) is evaluated. If empty, t = [], the
%           RenewalProcessPMF returns pmf as anonymous function of time t,
%           pmf = @(t) pmf(t);
