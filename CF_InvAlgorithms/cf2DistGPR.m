function [result,cdf,pdf,qf] = cf2DistGPR(cf,x,prob,options)
%cf2DistGPR Calculates the CDF/PDF/QF from the characteristic function CF
% by using the Gil-Pelaez inversion formulae using Riemann sum (instead of
% trapezium rule), as suggested in Shepard (1991). The algorithm is
% a minor adaptation from cf2DistGPT. 
%
...
%  QUADRATURE method, see [6].
%
% SYNTAX:
%  result = cf2DistGPR(cf,x)
...
%
% REMARKS:
% If options.DIST is provided, then cf2DistGPR evaluates CDF/PDF based on
...
%  result = cf2DistGPR(cf,x,prob,options)
%
...
%     39(1-2), pp.37-46.   
%
% SEE ALSO: cf2Dist, cf2DistGP, cf2DistGPR, cf2DistGPA, cf2DistFFT,
...
if any(x < xMin) || any(x > xMax)
    warning('VW:CharFunTool:cf2DistGPR',['x out-of-range: ', ...
...
%cdf     = (xMean - x)/2 + imag(E * (cft ./ t)); % cf2DistGPT
cdf     = imag(E * (cft ./ t));
cdf     = 0.5 - (cdf * dt) / pi;
...
if options.isInterp
    ...
    if ~xempty
        ...
        warning('VW:CharFunTool:cf2DistGPR', ...
