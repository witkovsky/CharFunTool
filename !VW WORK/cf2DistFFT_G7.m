function [result,cdf,pdf,qf] = cf2DistFFT_G7(cf,x,prob,options)
%cf2DistFFT_G7(cf,x,prob,options) Evaluates the CDF/PDF/QF (quantiles) from
% the characteristic function CF of a (continuous) distribution F by using
% the Fast Fourier Transform (FFT) algorithm applied with Gauss quadrature
% rule. 
%
% The algorithm cf2DistFFT_G7 evaluates the approximate values CDF(x), PDF(x),
...
%
% SYNTAX:
%  result = cf2DistFFT_G7(cf,x,prob,options)
%  [result,cdf,pdf,qf] = cf2DistFFT_G7(cf,x,prob,options)
%
% INPUT:
%  cf      - function handle for the characteristic function CF,
%  x       - vector of values from domain of the distribution F, if x = [],
%            cf2DistFFT_G7 automatically selects vector x from the domain.
%  prob    - vector of values from [0,1] for which the quantiles will be
%            estimated, if prob = [], cf2DistFFT_G7 automatically selects
...
%  options.xMin = -50;
%  options.xMax = 50;
%  result = cf2DistFFT_G7(cf_Y,[],[],options);
...
%
% REMARKS:
%  The outputs of the algorithm cf2DistFFT_G7 are approximate values! The
...
for id = 1:nRule
    t      = (k - N/2 + bp(id)) * dt;
    cftP   = cf(t(N/2+1:end));
    cft    = [conj(cftP(end:-1:1));cftP];
    C      = ((-1).^((A/(B-A) + k/N)*(N-2*bp(id))))/(B-A);
    pdfFFT = pdfFFT + w(id)*real(C.*fft(D.*cft));
    cdfFFT = cdfFFT + w(id)*real(1i*C.*fft(D.*cft./t));
end
pdfFFT = max(0,pdfFFT);
cdfFFT = min(1,max(0,0.5 + cdfFFT));
xFFT   = A + k * dx;
...
try
    ...
    warning('cf2DistFFT_G7: Unable to interpolate the required PDF values')
    pdf = NaN*x;
end
x       = reshape(x,szx);
