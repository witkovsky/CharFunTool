function [result,cdf,pdf,qf] = cf2DIST(cf,x,prob,options)
%cf2DIST Calculates the CDF/PDF/QF from the characteristic function CF
% by using the Gil-Pelaez inversion formulae and the simple trapezoidal
...
% SYNTAX:
%  result = cf2DIST(cf,x)
%  [result,cdf,pdf,qf] = cf2DIST(cf,x,prob,options)
%
% INPUT:
...
%
% REMARKS:
% If options.DIST is provided, then cf2DIST evaluates CDF/PDF based on
...
%  df   = 1:10;
%  ncp  = 0.1:0.1:1;
%  cf   = @(t) cf_ChiSquare(t,coef,df,ncp);
%  clear options
%  options.xMin = 0;
%  options.N = 2^12;
...
%
% EXAMPLE2 (cont.)
%  %% Get the estimated interpolation functions for CDF/PDF/QF/RND 
%  %% and use them for computing next (vector) values of CDF/PDF/QF or RND
%  PDF = result.PDF; f = PDF(0:10:1000)
%  CDF = result.CDF; F = CDF(0:10:1000)
%  QF  = result.QF; q = QF(0.8:0.005:0.999)
%  RND = result.RND; n = 10, r = RND(n)
%
% REFERENCES:
% [1] IMHOF, J.: Computing the distribution of quadratic forms in normal
...
%
% DEVELOPMENT:
% A version of the algorithm cf2DIST (cf2DistGP) is an integral part of CF
% TOOLBOX (The Characteristic Functions Toolbox), a MATLAB toolbox under
% development, see https://github.com/witkovsky/Characteristic-functions/.
% A simplified version of the TOOLBOX for R programming language (CharFun
% package) is under development at https://github.com/Simkova/CharFun/. A
% working version of the toolbox (with MATLAB p-code functions) is
% available at the web page, https://goo.gl/gBfdwY.
...
if any(x < xMin) || any(x > xMax)
    warning('VW:CFTOOLBOX:cf2DIST',['x out-of-range (the used support): ', ...
