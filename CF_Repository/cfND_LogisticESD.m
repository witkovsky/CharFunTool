...
%% cfND_LogisticESD  
%  Characteristic function of N-VARIATE LOGISTIC random variable with
%  ELLIPTICALLY SYMMETRIC DISTRIBUTION (ESD), say X = (X1,...,XN), with
...
%  N-variate normal random vector with mean 0 and variance–covariance
%  matrix Sigma, U is a Kolmogorov–Smirnov random variable, Z and U are
%  independent, and μ is an N-valued (non-random) vector.
%
...
%  as X = 1/sqrt(E(U^2))*U*Z + μ, where Z is an N-variate normal random
...
%  N-valued (non-random) vector. Here E(X) = μ and cov(X, X) = Sigma.
% 
% SYNTAX:
...
%
% EXAMPLE 2:
% % CF of the 3-variate Logistic RV
%   mu    = [1 2 3]';
...
%
% EXAMPLE 3:
% % CF of convolution of 3 3-variate Logistic Distributions
%   mu1    = [1 2 3]';
...
%
% EXAMPLE 4:
% % CF of convolution of 3 bivariate Logistic Distributions
%   mu1    = [1 2]';
%   Sigma1 = [1 0.5; 0.5 1];
...
TT = sqrt(3*sum(t*Sigma.*t,2));
cf = exp(1i*t*mu(:)) .* (TT ./ sinh(TT));
cf(TT==0) = 1;
