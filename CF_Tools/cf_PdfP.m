function [cf,coefs] = cf_PdfP(t,pdfFun,A,B,nPts)
%cf_PdfP 
%  Computes the characteristic function of the continuos distribution from
%  its probability density function (PDF) by using the PATTERSON method for
%  computing the Fourier Integral over the interval (A,B). pdfFun is
%  function handle for computing the PDF at arbitrary x in (A,B).
%
...
%  must be finite. 
%
%  cf_PdfP is evaluated from the standard integral representation of the
...
%
% REMARK:
%  cf_PdfP is suggested for situations when the PDF can be well approximated
...
%
% SYNTAX:
%  [cf,coefs] = cf_PdfP(t,pdfFun,A,B,nPts)
%
% INPUTS:
...
pr = (abs(FourierIntegral_P(-1e-8,pdfFun,A,B,nPts)) + ...
    abs(FourierIntegral_P(1e-8,pdfFun,A,B,nPts)))/2;
